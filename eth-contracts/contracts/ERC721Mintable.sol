...
contract Ownable {
    ...
    event OwnershipChanged(address newOwner, uint256 timestamp);
    ...
    function transferOwnership(address newOwner, uint256 timestamp) public onlyOwner {
        // TODO add functionality to transfer control of the contract to a newOwner.
        // make sure the new owner is a real address
        require(!Address.isContract(newOwner), "transferOwnerShip: newOwner isn't a real address");

        //Change contract owner
        _owner = newOwner;

        //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
        emit OwnershipChanged(newOwner, timestamp);
    }
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        address ownerOfToken = _tokenOwner[tokenId];
        require(ownerOfToken != to, "approve: address is owner of tokenId!");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(_tokenOwner[tokenId] != msg.sender || isApprovedForAll(ownerOfToken, to), "approve: token msg sender isnt owner!");
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        ...
        address tokenApproval = _tokenApprovals[tokenId];
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId),"transferFrom: isnt ApprovedOrOwner!");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!_exists(tokenId), "_mint: token does not exist!");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "transferFrom tokenOwner: token does not exist!");

        // TODO: require token is being transfered to valid address
        require(to != address(0), "TransferFrom: newOwner isn't a real address");
